...
class PaymentNoticesServiceImplTest {
    ...
    void givenWhenRetrieveGeneratedNoticeThen() {
        ...
        Mockito.when(gpdServiceMock.generatePaymentNoticeFromGPD(iamUserInfoDTO,paymentNoticePayloadDTO)).thenReturn(paymentNoticeResponseDTO);
        //when
        PaymentNoticeResponseDTO result = paymentNoticesService.retrieveGeneratedNotice(iamUserInfoDTO, paymentNoticePayloadDTO);

        //then
        Assertions.assertNotNull(result);
        Assertions.assertEquals(paymentNoticeResponseDTO, result);
        Assertions.assertTrue(memoryAppender.getLoggedEvents().get(0).getFormattedMessage().contains("[POST_GENERATE_PAYMENT_NOTICE] User ID user_id filled out a form to create a spontaneous payment, a payment notice was generated by GPD for EC ORGANIZATION_NAME with tax code ORGANIZATION_FISCAL_CODE"));
    }
}